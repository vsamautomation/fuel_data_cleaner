"""
Step 2: Extract ULLAGE Readings for September 2025
Extract ullage (empty space) readings for multiple sites across all tanks
"""
...
def extract_site_ullage(df, site_row, site_name, date_columns):
    """Extract ullage readings for a single site"""
    print(f"\n{'='*60}")
    print(f"Extracting: {site_name} (starting at row {site_row})")
    print('='*60)
    
    # Find the ULLAGE section by looking for "ULLAGE" label in column 3
    # Then collect product rows that follow until we hit another section
    
    ullage_start_row = None
    ullage_end_row = None
    
    # Search for ULLAGE section (within ~30 rows of site header)
    for offset in range(30):
        ...
        if "ULLAGE" in section_label.upper():
            ullage_start_row = row_idx
            print(f"  Found ULLAGE section at row {row_idx}")
            break
    
    if ullage_start_row is None:
        print(f"  WARNING: Could not find ULLAGE section for {site_name}")
        return []
    
    # Now find where ULLAGE section ends (next section like LOADS, SALES, etc.)
    for offset in range(1, 15):
        row_idx = ullage_start_row + offset
        if row_idx >= len(df):
            break
        
        section_label = str(df.iloc[row_idx, 3]).strip() if pd.notna(df.iloc[row_idx, 3]) else ""
        
        if any(keyword in section_label.upper() for keyword in ['LOADS', 'SALES', 'PROJECTED', 'ACTUAL', 'CARRIER']):
            ullage_end_row = row_idx
            print(f"  ULLAGE section ends at row {row_idx} (next section: {section_label})")
            break
    
    if ullage_end_row is None:
        ullage_end_row = ullage_start_row + 10  # Default to 10 rows
    
    # Scan the ULLAGE section to find all product entries
    records = []
    products_found = {}
    
    for row_idx in range(ullage_start_row, ullage_end_row):
        if row_idx >= len(df):
            break
    ...
    # Now extract ullage values for each date
    for col_idx, date in date_columns:
        for product, row_indices in products_found.items():
            # Create a record with multiple tank ullage readings
            record = {
            ...
            for tank_num, row_idx in enumerate(row_indices, start=1):
                ...
                if pd.notna(value):
                    try:
                        ...
                        record[f'Tank_{tank_num}_Ullage'] = numeric_val
                    except:
                        record[f'Tank_{tank_num}_Ullage'] = None
                else:
                    record[f'Tank_{tank_num}_Ullage'] = None
            
            records.append(record)
    
    print(f"  Extracted {len(records)} ullage records for {site_name}")
    return records
...
def main():
    """Main execution"""
    print("="*80)
    print("STEP 2: EXTRACT SEPTEMBER 2025 ULLAGE")
    print("="*80)
    ...
    # Step 3: Extract ullage for each site
    all_records = []
    
    for site_row, site_name in SITES:
        records = extract_site_ullage(df, site_row, site_name, sept_cols)
    ...
    df_output.to_excel(OUTPUT_FILE, index=False, sheet_name='Sept2025_Ullage')
